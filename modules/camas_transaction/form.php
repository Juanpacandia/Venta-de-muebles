...
<script type="text/javascript">
  function tampil_obat(input){
    ...
    $.post("modules/camas_transaction/camas.php", {
      dataidobat: num,
    }, function(response) {      
      $('#stok').html(response)
...
if ($_GET['form']=='add') { ?> 
  ...
  <section class="content-header">
    <h1>
      <i class="fa fa-edit icon-title"></i> Datos entradas / salidas de Camas
    </h1>
    <ol class="breadcrumb">
      <li><a href="?module=start"><i class="fa fa-home"></i> Inicio </a></li>
      <li><a href="?module=camas_transaction"> Entrada </a></li>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary">
          <!-- form start -->
          <form role="form" class="form-horizontal" action="modules/camas_transaction/proses.php?act=insert" method="POST" name="formObatMasuk">
            <div class="box-body">
              <?php  
            
              $query_id = mysqli_query($mysqli, "SELECT RIGHT(codigo_transaccion,7) as codigo_cama FROM transaccion_camas
              ...
              $tahun          = date("Y");
              $buat_id        = str_pad($codigo_cama, 7, "0", STR_PAD_LEFT);
              ...
              <div class="form-group">  
                <label class="col-sm-2 control-label">Cama</label>
                <div class="col-sm-5">
                  <select class="chosen-select" name="codigo_cama" data-placeholder="-- Seleccionar Cama --" onchange="tampil_obat(this)" autocomplete="off" required>
                    <option value=""></option>
                    <?php
                      $query_obat = mysqli_query($mysqli, "SELECT codigo_cama, tamaño FROM camas ORDER BY codigo_cama ASC")
                                                            or die('error '.mysqli_error($mysqli));
                      while ($data_obat = mysqli_fetch_assoc($query_obat)) {
                        echo"<option value=\"$data_obat[codigo_cama]\"> $data_obat[codigo_cama] | $data_obat[tamaño] </option>";
                      }
                    ?>
                  </select>
                </div>
            ...
            <div class="box-footer">
              <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                  ...
                  <a href="?module=camas_transaction" class="btn btn-default btn-reset">Cancelar</a>
                </div>
              </div>
            </div><!-- /.box footer -->
          </form>
