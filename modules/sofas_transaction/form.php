<script type="text/javascript">
  function tampil_obat(input){
    ...
    $.post("modules/sofas_transaction/sofas.php", {
      dataidobat: num,
    }, function(response) {      
      $('#stok').html(response)
...
if ($_GET['form']=='add') { ?> 
  ...
  <section class="content-header">
    <h1>
      <i class="fa fa-edit icon-title"></i> Datos entradas / salidas de Sofas
    </h1>
    <ol class="breadcrumb">
      <li><a href="?module=start"><i class="fa fa-home"></i> Inicio </a></li>
      <li><a href="?module=sofas_transaction"> Entrada </a></li>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary">
          <!-- form start -->
          <form role="form" class="form-horizontal" action="modules/sofas_transaction/proses.php?act=insert" method="POST" name="formObatMasuk">
            <div class="box-body">
              <?php  
            
              $query_id = mysqli_query($mysqli, "SELECT RIGHT(codigo_transaccion,7) as codigo_sofa FROM transaccion_sofas
              ...
              $tahun          = date("Y");
              $buat_id        = str_pad($codigo_sofa, 7, "0", STR_PAD_LEFT);
              ...
              <div class="form-group">  
                <label class="col-sm-2 control-label">Sofa</label>
                <div class="col-sm-5">
                  <select class="chosen-select" name="codigo_sofa" data-placeholder="-- Seleccionar Sofa --" onchange="tampil_obat(this)" autocomplete="off" required>
                    <option value=""></option>
                    <?php
                      $query_obat = mysqli_query($mysqli, "SELECT codigo_sofa, tamaño FROM sofas ORDER BY codigo_sofa ASC")
                                                            or die('error '.mysqli_error($mysqli));
                      while ($data_obat = mysqli_fetch_assoc($query_obat)) {
                        echo"<option value=\"$data_obat[codigo_sofa]\"> $data_obat[codigo_sofa] | $data_obat[tamaño] </option>";
                      }
                    ?>
                  </select>
                </div>
            ...
            <div class="box-footer">
              <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                  ...
                  <a href="?module=sofas_transaction" class="btn btn-default btn-reset">Cancelar</a>
                </div>
              </div>
            </div><!-- /.box footer -->
          </form>
